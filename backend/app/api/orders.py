...
def list_my_orders():
    ...
    orders, total = service.my_orders(user_id=get_jwt_identity(), page=page, per_page=per_page)
...
def list_all_orders():
    ...
    orders, total = service.list_orders(page=page, per_page=per_page, status=status)
    meta = {"page": page, "per_page": per_page, "total": total, "pages": (total + per_page - 1) // per_page}
    return success_response(data=orders, meta=meta)
